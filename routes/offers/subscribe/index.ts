...
import {
  ...
  SUBSCRIBE_FAIL,
  SUBSCRIBE_TOTAL_SUCCESS,
} from "../../../utils/flags";
import { PhoneNumbers } from "../../../utils/types";

export default (router: Router) => {
  router.options("/offers/:offerId/subscribe", async (_, res) => {
  ...
  router.post("/offers/:offerId/subscribe", async (req, res) => {
    ...
    if (SUBSCRIBE_FAIL) return res.status(500).send("Controlled error");
    if (SUBSCRIBE_TOTAL_SUCCESS) {
